...
unsigned screenWidth = 320;
unsigned screenHeight = 240;
unsigned screenBits = 16;      // use "best" color depth according to libSDL
#endif
...
void	VL_SetVGAPlaneMode (void)
{
    ...
    screen = SDL_SetVideoMode(320, 240, screenBits, 
        SDL_HWSURFACE | (screenBits == 16 ? SDL_HWPALETTE : 0) | (fullscreen ? SDL_FULLSCREEN : 0));
    // screen = SDL_CreateRGBSurface(SDL_SWSURFACE, screenWidth, screenHeight, screenBits, 0, 0, 0, 0);
    if(!screen)
    {
    ...
    // SDL_SetColors(ScreenSurface, gamepal, 0, 256);
...
void VL_SetColor	(int color, int red, int green, int blue)
{
    ...
    if(screenBits == 8){
        ...
        // SDL_SetPalette(ScreenSurface, SDL_PHYSPAL, &col, color, 1);
    }
    else
    {
...
void VL_SetPalette (SDL_Color *palette)
{
    ...
    if(screenBits == 8){
        ...
        // SDL_SetPalette(ScreenSurface, SDL_PHYSPAL, palette, 0, 256);
    }
    else
    {
